...
public class ChatService : DisposableMediatorSubscriberBase
{
    ...
    public ChatService(ILogger<ChatService> logger, DalamudUtilService dalamudUtil, ShibaBridgeMediator mediator, ApiController apiController,
        PairManager pairManager, ILoggerFactory loggerFactory, IGameInteropProvider gameInteropProvider, IChatGui chatGui,
        ShibaBridgeConfigService shibabridgeConfig, ServerConfigurationManager serverConfigurationManager) : base(logger, mediator)
    {
        _logger = logger;
    ...
    private ushort ResolveShellColor(int shellColor)
    {
        ...
        var globalColor = _shibabridgeConfig.Current.ChatColor;
        if (globalColor != 0)
    ...
    private XivChatType ResolveShellLogKind(int shellLogKind)
    {
        ...
        return (XivChatType)_shibabridgeConfig.Current.ChatLogKind;
    }

    private void HandleGroupChat(GroupChatMsgMessage message)
    {
        if (_shibabridgeConfig.Current.DisableSyncshellChat)
            return;
        ...
        var extraChatTags = _shibabridgeConfig.Current.ExtraChatTags;
    ...
    {
        int chatType = _shibabridgeConfig.Current.ChatLogKind;
    ...
    public void SwitchChatShell(int shellNumber)
    {
        ...
        _chatGui.PrintError($"[ShibaBridgeSync] Syncshell number #{shellNumber} not found");
    }
