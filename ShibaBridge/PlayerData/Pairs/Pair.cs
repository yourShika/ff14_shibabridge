...
public class Pair : DisposableMediatorSubscriberBase
{
    ...
    public Pair(ILogger<Pair> logger, UserData userData, PairHandlerFactory cachedPlayerFactory,
        ShibaBridgeMediator mediator, ShibaBridgeConfigService shibabridgeConfig, ServerConfigurationManager serverConfigurationManager)
    ...
    {
        string? note = GetNote();
        if (_shibabridgeConfig.Current.ShowCharacterNames || IsVisible)
