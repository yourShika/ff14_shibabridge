...
namespace ShibaBridge.Services.ServerConfiguration;
...
public class ServerConfigurationManager
{
    ...
    private void EnsureMainExists()
    {
        ...
        for (int i = 0; i < _configService.Current.ServerStorage.Count; ++i)
        {
            ...
            if (x.ServerUri.Equals(ApiController.ShibaBridgeServiceUri, StringComparison.OrdinalIgnoreCase))
                elfExists = true;
        }
        if (!elfExists)
        {
            _logger.LogDebug("Re-adding missing server {uri}", ApiController.ShibaBridgeServiceUri);
            _configService.Current.ServerStorage.Insert(0, new ServerStorage() { ServerUri = ApiController.ShibaBridgeServiceUri, ServerName = ApiController.ShibaBridgeServer });
            if (_configService.Current.CurrentServer >= 0)
                _configService.Current.CurrentServer++;
        }
        Save();
