...
public class FileTransferOrchestrator : DisposableMediatorSubscriberBase
{
    ...
    public FileTransferOrchestrator(ILogger<FileTransferOrchestrator> logger, ShibaBridgeConfigService shibabridgeConfig,
        ShibaBridgeMediator mediator, TokenProvider tokenProvider) : base(logger, mediator)
    {
        ...
        _availableDownloadSlots = shibabridgeConfig.Current.ParallelDownloads;
    ...
    public async Task WaitForDownloadSlotAsync(CancellationToken token)
    {
        ...
        {
            if (_availableDownloadSlots != _shibabridgeConfig.Current.ParallelDownloads && _availableDownloadSlots == _downloadSemaphore.CurrentCount)
            {
                _availableDownloadSlots = _shibabridgeConfig.Current.ParallelDownloads;
                _downloadSemaphore = new(_availableDownloadSlots, _availableDownloadSlots);
            }
        }
    ...
    {
        var limit = _shibabridgeConfig.Current.DownloadSpeedLimitInBytes;
        if (limit <= 0) return 0;
        limit = _shibabridgeConfig.Current.DownloadSpeedType switch
        {
            ShibaBridgeConfiguration.Models.DownloadSpeeds.Bps => limit,
            ShibaBridgeConfiguration.Models.DownloadSpeeds.KBps => limit * 1024,
            ShibaBridgeConfiguration.Models.DownloadSpeeds.MBps => limit * 1024 * 1024,
            _ => limit,
        };
