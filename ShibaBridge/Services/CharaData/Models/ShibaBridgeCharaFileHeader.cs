...
public record ShibaBridgeCharaFileHeader(byte Version, ShibaBridgeCharaFileData CharaFileData)
{
    ...
    public ShibaBridgeCharaFileData CharaFileData { get; set; } = CharaFileData;
    ...
    public static ShibaBridgeCharaFileHeader? FromBinaryReader(string path, BinaryReader reader)
    {
        var chars = new string(reader.ReadChars(4));
        if (!string.Equals(chars, "MCDF", StringComparison.Ordinal)) throw new InvalidDataException("Not a ShibaBridge Chara File");

        ShibaBridgeCharaFileHeader? decoded = null;
        ...
        if (version == 1)
        {
            ...
            decoded = new(version, ShibaBridgeCharaFileData.FromByteArray(reader.ReadBytes(dataLength)))
            {
                FilePath = path,
            };
        }
