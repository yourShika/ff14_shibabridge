// PlayerPerformanceService - part of ShibaBridge project.
...
public class PlayerPerformanceService : DisposableMediatorSubscriberBase
{
    ...
    public PlayerPerformanceService(ILogger<PlayerPerformanceService> logger, ShibaBridgeMediator mediator,
        ...
        if (triUsage > triUsageThreshold)
        {
            if (notify && !pair.IsApplicationBlocked)
            {
                _mediator.Publish(new NotificationMessage($"{pair.PlayerName} ({pair.UserData.AliasOrUID}) automatically blocked",
                    ...
                    ShibaBridgeConfiguration.Models.NotificationType.Warning));
            }
    ...
    public async Task<bool> ShrinkTextures(PairHandler pairHandler, CharacterData charaData, CancellationToken token)
    {
        ...
        if (config.TextureShrinkMode == ShibaBridgeConfiguration.Models.TextureShrinkMode.Never)
            return false;

        // XXX: Temporary
        if (config.TextureShrinkMode == ShibaBridgeConfiguration.Models.TextureShrinkMode.Default)
            return false;
