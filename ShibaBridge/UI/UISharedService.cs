// UISharedService - part of ShibaBridge project.
...
public partial class UiSharedService : DisposableMediatorSubscriberBase
{
    ...
    private const string _notesEnd = "##SHIBABRIDGE_USER_NOTES_END##";

    private const string _notesStart = "##SHIBABRIDGE_USER_NOTES_START##";
    ...
    public UiSharedService(ILogger<UiSharedService> logger, IpcManager ipcManager, ApiController apiController,
        ...
        ShibaBridgeConfigService configService, DalamudUtilService dalamudUtil, IDalamudPluginInterface pluginInterface,
        ITextureProvider textureProvider,
        ServerConfigurationManager serverManager, ShibaBridgeMediator mediator) : base(logger, mediator)
    {
    ...
    {
        ColorTextWrapped("Note: The storage folder should be somewhere close to root (i.e. C:\\ShibaBridgeStorage) in a new empty folder. DO NOT point this to your game folder. DO NOT point this to your Penumbra folder.", ImGuiColors.DalamudYellow);
        ...
        ImGui.SameLine();
        using (ImRaii.Disabled(_cacheMonitor.ShibaBridgeWatcher != null))
        {
            if (IconButton(FontAwesomeIcon.Folder))
            {
                FileDialogManager.OpenFolderDialog("Pick ShibaBridge Storage Folder", (success, path) =>
                {
                    if (!success) return;
                        ...
                        && _cacheDirectoryIsValidPath)
                    {
                        ...
                        _configService.Save();
                        _cacheMonitor.StartShibaBridgeWatcher(path);
                        _cacheMonitor.InvokeScan();
                    }
                }, _dalamudUtil.IsWine ? @"Z:\" : @"C:\");
            }
        }
        if (_cacheMonitor.ShibaBridgeWatcher != null)
        {
        ...
        {
            ColorTextWrapped("Your selected directory has files or directories inside that are not ShibaBridge related. Use an empty directory or a previous storage directory only.", ImGuiColors.DalamudRed);
        }
        ...
        DrawHelpText("The storage is automatically governed by ShibaBridge. It will clear itself automatically once it reaches the set capacity by removing the oldest unused files. You typically do not need to clear it yourself.");
    }
    ...
    public bool DrawOtherPluginState(bool intro = false)
    {
        ...
        {
            ImGui.TextColored(ImGuiColors.DalamudRed, "You need to install both Penumbra and Glamourer and keep them up to date to use ShibaBridge.");
            return false;
        }
