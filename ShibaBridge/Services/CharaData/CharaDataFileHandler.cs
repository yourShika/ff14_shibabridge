...
public sealed class CharaDataFileHandler : IDisposable
{
    ...
    private readonly ShibaBridgeCharaFileDataFactory _shibabridgeCharaFileDataFactory;
            ...
            DalamudUtilService dalamudUtilService, GameObjectHandlerFactory gameObjectHandlerFactory, PlayerDataFactory playerDataFactory)
    {
        ...
        _shibabridgeCharaFileDataFactory = new(fileCacheManager);
    }
    ...
    public Task<(ShibaBridgeCharaFileHeader loadedCharaFile, long expectedLength)> LoadCharaFileHeader(string filePath)
    {
        try
        {
            ...
            var loadedCharaFile = ShibaBridgeCharaFileHeader.FromBinaryReader(filePath, reader);

            _logger.LogInformation("Read ShibaBridge Chara File");
    ...
    public Dictionary<string, string> McdfExtractFiles(ShibaBridgeCharaFileHeader? charaFileHeader, long expectedLength, List<string> extractedFiles)
    {
        ...
        using var reader = new BinaryReader(lz4Stream);
        ShibaBridgeCharaFileHeader.AdvanceReaderToData(reader);

        long totalRead = 0;
        Dictionary<string, string> gamePathToFilePath = new(StringComparer.Ordinal);
        foreach (var fileData in charaFileHeader.CharaFileData.Files)
        {
            var fileName = Path.Combine(_fileCacheManager.CacheFolder, "shibabridge_" + _globalFileCounter++ + ".tmp");
    ...
    internal async Task SaveCharaFileAsync(string description, string filePath)
    {
        ...
        try
        {
            ...
            var shibabridgeCharaFileData = _shibabridgeCharaFileDataFactory.Create(description, data);
            ShibaBridgeCharaFileHeader output = new(ShibaBridgeCharaFileHeader.CurrentVersion, shibabridgeCharaFileData);
        ...
        {
            _logger.LogError(ex, "Failure Saving ShibaBridge Chara File, deleting output");
