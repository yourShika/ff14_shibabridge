...
namespace ShibaBridge.UI.Components;
...
public class PairGroupsUi
{
    ...
    public PairGroupsUi(ShibaBridgeConfigService shibabridgeConfig, TagHandler tagHandler, UidDisplayHandler uidDisplayHandler, ApiController apiController,
    ...
    private void DrawName(string tag, bool isSpecialTag, int visible, int online, int? total)
    {
        string displayedName = tag switch
        {
            ...
            TagHandler.CustomOnlineTag => _shibabridgeConfig.Current.ShowOfflineUsersSeparately ? "Online/Paused" : "Contacts",
    ...
    {
        if (_shibabridgeConfig.Current.ShowVisibleUsersSeparately)
        {
        ...
        {
            if (_shibabridgeConfig.Current.ShowOfflineUsersSeparately)
            {
