// PopoutProfileUi - part of ShibaBridge project.
...
public class PopoutProfileUi : WindowMediatorSubscriberBase
{
    ...
    public PopoutProfileUi(ILogger<PopoutProfileUi> logger, ShibaBridgeMediator mediator, UiSharedService uiSharedService,
        ServerConfigurationManager serverManager, ShibaBridgeConfigService shibabridgeConfigService,
        ShibaBridgeProfileManager shibabridgeProfileManager, PairManager pairManager, PerformanceCollectorService performanceCollectorService) : base(logger, mediator, "###ShibaBridgeSyncPopoutProfileUI", performanceCollectorService)
    {
        ...
        Mediator.Subscribe<CompactUiChange>(this, (msg) =>
        {
            ...
            if (shibabridgeConfigService.Current.ProfilePopoutRight)
            {
    ...
    protected override void DrawInternal()
    {
        ...
        try
        {
            ...
            var shibabridgeProfile = _shibabridgeProfileManager.GetShibaBridgeProfile(_pair.UserData);

            if (_textureWrap == null || !shibabridgeProfile.ImageData.Value.SequenceEqual(_lastProfilePicture))
            {
                _textureWrap?.Dispose();
                _lastProfilePicture = shibabridgeProfile.ImageData.Value;
            ...
            var descText = shibabridgeProfile.Description;
            ...
            UiSharedService.TextWrapped(trimmed ? descText + $"...{Environment.NewLine}[Open Full Profile for complete description]" : shibabridgeProfile.Description);
