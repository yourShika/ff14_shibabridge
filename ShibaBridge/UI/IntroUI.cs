// IntroUI - part of ShibaBridge project.
...
public partial class IntroUi : WindowMediatorSubscriberBase
{
    ...
    public IntroUi(ILogger<IntroUi> logger, UiSharedService uiShared, ShibaBridgeConfigService configService,
        CacheMonitor fileCacheManager, ServerConfigurationManager serverConfigurationManager, ShibaBridgeMediator shibabridgeMediator,
        PerformanceCollectorService performanceCollectorService, DalamudUtilService dalamudUtilService, AccountRegistrationService registerService) : base(logger, shibabridgeMediator, "ShibaBridge Setup", performanceCollectorService)
    {
        _uiShared = uiShared;
    ...
    protected override void DrawInternal()
    {
        ...
        {
            _uiShared.BigText("Welcome to ShibaBridge");
            ImGui.Separator();
            UiSharedService.TextWrapped("ShibaBridge is a plugin that will replicate your full current character state including all Penumbra mods to other paired users. " +
        ...
        else if (!_configService.Current.AcceptedAgreement && _readFirstPage)
        {
            ...
            UiSharedService.TextWrapped("""
                                        ...
                                        """);
            UiSharedService.TextWrapped("""
                                        ShibaBridge is operated from servers located in the European Union. You agree not to upload any content to the service that violates EU law; and more specifically, German law.
                                        """);
            UiSharedService.TextWrapped("""
                     ...
                     || !Directory.Exists(_configService.Current.CacheFolder)))
        {
            ...
            {
                UiSharedService.TextWrapped("To not unnecessary download files already present on your computer, ShibaBridge will have to scan your Penumbra mod directory. " +
                                     "Additionally, a local storage folder must be set where ShibaBridge will download other character files to. " +
                                     "Once the storage folder is set and the scan complete, this page will automatically forward to registration at a service.");
                UiSharedService.TextWrapped("Note: The initial scan, depending on the amount of mods you have, might take a while. Please wait until it is completed.");
                UiSharedService.ColorTextWrapped("Warning: once past this step you should not delete the FileCache.csv of ShibaBridge in the Plugin Configurations folder of Dalamud. " +
            ...
            if (!_dalamudUtilService.IsWine)
            {
                ...
                UiSharedService.ColorTextWrapped("The File Compactor can save a tremendeous amount of space on the hard disk for downloads through ShibaBridge. It will incur a minor CPU penalty on download but can speed up " +
                    "loading of other characters. It is recommended to keep it enabled. You can change this setting later anytime in the ShibaBridge settings.", ImGuiColors.DalamudYellow);
            }
        }
        else if (!_uiShared.ApiController.IsConnected)
        {
            ...
            ImGui.Separator();
            UiSharedService.TextWrapped("To be able to use ShibaBridge you will have to register an account.");
            ...
            if (true) // Enable registration button for all servers
            {
                ...
                ImGui.Separator();
                ImGui.TextUnformatted("If you have not used ShibaBridge before, click below to register a new account.");
