// UidDisplayHandler - part of ShibaBridge project.
...
public class UidDisplayHandler
{
    ...
    public UidDisplayHandler(ShibaBridgeMediator mediator, PairManager pairManager,
    ...
    public void DrawPairText(string id, Pair pair, float textPosX, float originalY, Func<float> editBoxWidth)
    {
        ...
        if (!string.Equals(_editNickEntry, pair.UserData.UID, StringComparison.Ordinal))
        {
            ...
            if (ImGui.IsItemHovered())
            {
                ...
                {
                    _popupTime = DateTime.UtcNow.AddSeconds(_shibabridgeConfigService.Current.ProfileDelay);
                }

                _lastMouseOverUid = id;

                if (_popupTime > DateTime.UtcNow || !_shibabridgeConfigService.Current.ProfilesShow)
                {
    ...
    public (bool isUid, string text) GetPlayerText(Pair pair)
    {
        ...
        if (_shibabridgeConfigService.Current.ShowCharacterNames && textIsUid && !showUidInsteadOfName)
        {
