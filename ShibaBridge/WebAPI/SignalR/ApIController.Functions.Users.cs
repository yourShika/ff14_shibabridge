...
public partial class ApiController
{
    ...
    {
        if (!IsConnected) return;
        await _shibabridgeHub!.SendAsync(nameof(UserAddPair), user).ConfigureAwait(false);
    }

    public async Task UserChatSendMsg(UserDto user, ChatMessage message)
    {
        CheckConnection();
        await _shibabridgeHub!.SendAsync(nameof(UserChatSendMsg), user, message).ConfigureAwait(false);
    }

    public async Task UserDelete()
    {
        CheckConnection();
        await _shibabridgeHub!.SendAsync(nameof(UserDelete)).ConfigureAwait(false);
        await CreateConnections().ConfigureAwait(false);
    }

    public async Task<List<OnlineUserIdentDto>> UserGetOnlinePairs()
    {
        return await _shibabridgeHub!.InvokeAsync<List<OnlineUserIdentDto>>(nameof(UserGetOnlinePairs)).ConfigureAwait(false);
    }

    public async Task<List<UserPairDto>> UserGetPairedClients()
    {
        return await _shibabridgeHub!.InvokeAsync<List<UserPairDto>>(nameof(UserGetPairedClients)).ConfigureAwait(false);
    }
    ...
    public async Task<UserProfileDto> UserGetProfile(UserDto dto)
    {
        ...
        return await _shibabridgeHub!.InvokeAsync<UserProfileDto>(nameof(UserGetProfile), dto).ConfigureAwait(false);
    }
    ...
    public async Task UserPushData(UserCharaDataMessageDto dto)
    {
        try
        {
            await _shibabridgeHub!.InvokeAsync(nameof(UserPushData), dto).ConfigureAwait(false);
        }
        catch (Exception ex)
        {
    ...
    {
        if (!IsConnected) return;
        await _shibabridgeHub!.SendAsync(nameof(UserRemovePair), userDto).ConfigureAwait(false);
    }

    public async Task UserReportProfile(UserProfileReportDto userDto)
    {
        if (!IsConnected) return;
        await _shibabridgeHub!.SendAsync(nameof(UserReportProfile), userDto).ConfigureAwait(false);
    }

    public async Task UserSetPairPermissions(UserPermissionsDto userPermissions)
    {
        await _shibabridgeHub!.SendAsync(nameof(UserSetPairPermissions), userPermissions).ConfigureAwait(false);
    }

    public async Task UserSetProfile(UserProfileDto userDescription)
    {
        if (!IsConnected) return;
        await _shibabridgeHub!.InvokeAsync(nameof(UserSetProfile), userDescription).ConfigureAwait(false);
    }
