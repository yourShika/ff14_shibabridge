...
using NotificationType = ShibaBridge.ShibaBridgeConfiguration.Models.NotificationType;
...
public class NotificationService : DisposableMediatorSubscriberBase, IHostedService
{
    ...
    public NotificationService(ILogger<NotificationService> logger, ShibaBridgeMediator mediator,
        DalamudUtilService dalamudUtilService,
        INotificationManager notificationManager,
        IChatGui chatGui, ShibaBridgeConfigService configurationService) : base(logger, mediator)
    {
    ...
    {
        SeStringBuilder se = new SeStringBuilder().AddText("[ShibaBridgeSync] Error: " + message);
        _chatGui.PrintError(se.BuiltString);
    }

    private void PrintInfoChat(string? message)
    {
        SeStringBuilder se = new SeStringBuilder().AddText("[ShibaBridgeSync] Info: ").AddItalics(message ?? string.Empty);
        _chatGui.Print(se.BuiltString);
    }

    private void PrintWarnChat(string? message)
    {
        SeStringBuilder se = new SeStringBuilder().AddText("[ShibaBridgeSync] ").AddUiForeground("Warning: " + (message ?? string.Empty), 31).AddUiForegroundOff();
        _chatGui.Print(se.BuiltString);
    }
