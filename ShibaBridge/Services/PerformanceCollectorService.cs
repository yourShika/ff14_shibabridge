...
public sealed class PerformanceCollectorService : IHostedService
{
    ...
    public PerformanceCollectorService(ILogger<PerformanceCollectorService> logger, ShibaBridgeConfigService shibabridgeConfigService)
    {
        _logger = logger;
        _shibabridgeConfigService = shibabridgeConfigService;
    }

    public T LogPerformance<T>(object sender, ShibaBridgeInterpolatedStringHandler counterName, Func<T> func, int maxEntries = 10000)
    {
        if (!_shibabridgeConfigService.Current.LogPerformance) return func.Invoke();
    ...
    public void LogPerformance(object sender, ShibaBridgeInterpolatedStringHandler counterName, Action act, int maxEntries = 10000)
    {
        if (!_shibabridgeConfigService.Current.LogPerformance) { act.Invoke(); return; }
    ...
    {
        if (!_shibabridgeConfigService.Current.LogPerformance)
        {
